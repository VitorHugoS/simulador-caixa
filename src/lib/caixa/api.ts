...
const _simInflight = new Map<string, Promise<CaixaSimulacaoData>>()
...
export async function fetchSimulacao(
  ...
  // 1. Cache hit (resultado já resolvido)
  const hit = _simCache.get(cacheKey)
  if (hit && Date.now() - hit.ts < SIM_TTL) return hit.data

  // 2. Inflight dedup — se já existe uma Promise em voo para esta chave,
  //    retorna a mesma ao invés de disparar uma segunda requisição
  const inflight = _simInflight.get(cacheKey)
  if (inflight) return inflight

  // 3. Registra a Promise no mapa de inflight antes de iniciá-la
  ...
  // Registra e remove automaticamente ao resolver/rejeitar
  _simInflight.set(cacheKey, fetchPromise)
  fetchPromise.finally(() => _simInflight.delete(cacheKey))
  return fetchPromise
}
